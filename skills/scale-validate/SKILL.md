...
1. Run `scripts/validate-scale-agents.sh` and `scripts/validate-scale-library.sh` from the canonical repository. For a machine-specific model check, also run `node scripts/validate-scale-model-registry.mjs --catalog "$HOME/.codex/models.json" --config "$HOME/.codex/config.toml"` without exposing credentials.
...
4. Treat missing profiles, skills, registry metadata, unavailable providers, or unsupported models as findings. Do not promote changes until they are resolved or explicitly accepted.
