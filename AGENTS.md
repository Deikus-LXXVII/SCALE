...
- Codex profiles keep Terra for production frontend, hardware/domain integration, and sensitive implementation; OpenCode Go owns bounded routine/simple work and non-sensitive standard implementation. Native Spark medium is limited to bounded, non-sensitive small fixes, localized refactoring, boilerplate generation, single-file analysis, fast iterations, and granular UI changes; Spark low/read-only remains limited to passive test observation. Native Luna high remains the shared fallback, advisory, and independent-QA lane for broader or higher-context work. Sol remains critical authority. Every DeepSeek V4 Flash assignment is an OpenCode Go route; the DeepSeek API is not configured or used.
