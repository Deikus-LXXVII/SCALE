...
To activate a catalogued role in a connected project, use `scripts/scale-agent-activate.sh`. It symlinks the profile into the target project's `.codex/agents/` without overwriting a project-owned profile. For a core role that has no differentiated catalog note, the script resolves the matching active profile from `.codex/scale-library-src/.codex/agents/` instead of treating the catalog as complete.
