...
usage() {
  printf '%s\n' 'Usage: cell-agent-activate.sh --agent <name> --target <project-dir> [--library <library-dir>]'
}
...
library_root="${library_input:-$target_root/.codex/cell-library}"
...
if [[ "$library_root" == "$target_root/.codex/cell-library" ]]; then
  link_target="../cell-library/agents/$agent_name.toml"
else
  link_target="$profile"
fi
