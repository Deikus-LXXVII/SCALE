...
deepseek_count=$( (rg -l '^model = "deepseek-v4-flash"\r?$' "$agents_dir" || true) | wc -l | tr -d ' ')
if [[ "$deepseek_count" -ne 0 ]]; then
  echo "DeepSeek API model must not appear in Codex profiles; found $deepseek_count." >&2
  exit 1
fi

if ! rg -q '^model = "gpt-5.6-luna"\r?$' "$agents_dir/scale_orchestrator.toml" || ! rg -q '^model_reasoning_effort = "high"\r?$' "$agents_dir/scale_orchestrator.toml"; then
...
if ! rg -q '^model = "gpt-5.3-codex-spark"\r?$' "$agents_dir/scale_code_simple.toml" || ! rg -q '^model_reasoning_effort = "medium"\r?$' "$agents_dir/scale_code_simple.toml" || ! rg -q '^sandbox_mode = "workspace-write"\r?$' "$agents_dir/scale_code_simple.toml"; then
  echo "scale_code_simple Codex card must use its native Spark/medium/workspace-write fallback." >&2
  exit 1
fi
if ! rg -q '^model = "gpt-5.3-codex-spark"\r?$' "$agents_dir/scale_test_observer.toml" || ! rg -q '^model_reasoning_effort = "low"\r?$' "$agents_dir/scale_test_observer.toml" || ! rg -q '^sandbox_mode = "read-only"\r?$' "$agents_dir/scale_test_observer.toml"; then
...
for profile in scale_cleaner scale_environment scale_indexer scale_library; do
  if ! rg -q '^sandbox_mode = "read-only"\r?$' "$agents_dir/$profile.toml"; then
