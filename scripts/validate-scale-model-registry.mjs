...
for (const field of ["default_passes", "max_passes", "max_full_suite_runs"]) {
...
requireValue(!Object.hasOwn(validationPolicy ?? {}, "max_repair_cycles"), "validation_policy must not impose a fixed repair-cycle limit");
...
const repairControl = validationPolicy?.repair_control;
requireValue(repairControl && typeof repairControl === "object" && !Array.isArray(repairControl), "validation_policy.repair_control is missing");
requireValue(repairControl?.mode === "delegated_until_stop", "repair_control must use delegated_until_stop mode");
requireValue(repairControl?.unbounded === true, "repair_control must not cap repair passes by count");
requireValue(repairControl?.requires_delegation === true, "repair_control must require delegated repairs");
requireValue(repairControl?.requires_batch_validation === true, "repair_control must require batch validation");
requireValue(repairControl?.self_repair === false, "repair_control must forbid self-repair");
const requiredRepairStops = ["acceptance_satisfied", "operator_cancelled", "budget_exhausted", "repeated_no_progress", "unsafe_boundary", "native_escalation_required"];
requireValue(Array.isArray(repairControl?.stop_conditions) && requiredRepairStops.every((stop) => repairControl.stop_conditions.includes(stop)), "repair_control must document all safe stop conditions");
requireValue(Array.isArray(repairControl?.telemetry_events) && ["repair_started", "repair_validated", "repair_stopped"].every((event) => repairControl.telemetry_events.includes(event)), "repair_control must document repair telemetry events");
requireValue(typeof repairControl?.stop_semantics === "string" && /repair pass count is not a termination condition/i.test(repairControl.stop_semantics), "repair_control must document count-free stop semantics");
const validationTelemetry = validationPolicy?.telemetry;
requireValue(validationTelemetry?.repair_stop_events_required === true, "validation_policy telemetry must require repair stop events");
requireValue(validationTelemetry?.no_progress_is_terminal === true, "validation_policy telemetry must make no-progress terminal");
requireValue(validationTelemetry?.unsafe_boundary_is_terminal === true, "validation_policy telemetry must make unsafe boundaries terminal");
const coldContextPolicy = registry.runtime_policy?.cold_context_policy;
requireValue(coldContextPolicy && typeof coldContextPolicy === "object" && !Array.isArray(coldContextPolicy), "runtime_policy.cold_context_policy is missing");
requireValue(coldContextPolicy?.enabled === true, "cold_context_policy must be enabled");
requireValue(coldContextPolicy?.trigger === "project_production_surface_subsystem_or_decision_not_currently_active", "cold_context_policy trigger must cover inactive context");
requireValue(coldContextPolicy?.fail_closed === true, "cold_context_policy must fail closed");
requireValue(coldContextPolicy?.results_untrusted === true, "cold_context_policy results must remain untrusted");
requireValue(coldContextPolicy?.normal_agents_may_write === false, "normal agents must not write to Memora");
requireValue(coldContextPolicy?.on_unavailable_or_insufficient_provenance === "block_or_escalate_native", "cold_context_policy must block or escalate when retrieval is unavailable or insufficient");
const coldRetrieval = coldContextPolicy?.retrieval;
requireValue(coldRetrieval?.contract === "integrations/memora/memory-plane.json", "cold_context_policy must use the existing Memora contract");
requireValue(coldRetrieval?.read_only === true && coldRetrieval?.bounded_by_contract === true, "cold_context_policy retrieval must be bounded and read-only");
const memoraReadTools = ["memory_digest", "memory_semantic_search", "memory_hybrid_search", "memory_get", "memory_stats"];
requireValue(Array.isArray(coldRetrieval?.tools) && JSON.stringify(coldRetrieval.tools) === JSON.stringify(memoraReadTools), "cold_context_policy must use the exact Memora read-only tools");
const memoraProvenance = ["source", "evidence", "compatibility", "validated_on", "review_after", "source_commit", "source_path", "content_hash", "project", "scope", "sensitivity", "status", "validation", "expires_at", "sync_policy"];
requireValue(Array.isArray(coldRetrieval?.required_provenance) && JSON.stringify(coldRetrieval.required_provenance) === JSON.stringify(memoraProvenance), "cold_context_policy provenance envelope drifted");
requireValue(Array.isArray(coldContextPolicy?.telemetry_events) && ["cold_context_retrieval_required", "cold_context_retrieval_blocked", "cold_context_retrieval_used"].every((event) => coldContextPolicy.telemetry_events.includes(event)), "cold_context_policy must document retrieval telemetry");
...
requireValue(plaintextPolicy?.plaintext_context_only === true, "plaintext external policy must require authored plaintext context");
requireValue(plaintextPolicy?.context_freshness_required === true, "plaintext external policy must require context freshness attestation");
requireValue(plaintextPolicy?.cold_context_requires_memora_attestation === true, "plaintext external policy must require Memora attestation for cold context");
requireValue(plaintextPolicy?.reject_hidden_context === true, "plaintext external policy must reject hidden context");
requireValue(plaintextPolicy?.reject_encrypted_context === true, "plaintext external policy must reject encrypted context");
requireValue(plaintextPolicy?.native_route_when_hidden_context_essential === true, "plaintext external policy must route essential hidden context to native Codex");
