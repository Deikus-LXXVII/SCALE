...
For a multi-step task, invoke the `scale-orchestrator` skill. The main session
agent delegates implementation to one bounded executor by default, keeps edits
non-overlapping, and performs one batched acceptance check. Parallel executors
are reserved for independent scopes; repairs are delegated rather than silently
